...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match result{
        ...
        Result::Err(TicketNewError::TitleError(errmsg)) => panic!("{errmsg}"),
        Result::Err(TicketNewError::DescriptionError(errmsg)) => Ticket::new(title.clone(), "Description not provided".to_string(), status.clone()).unwrap()
    }
}
