...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, RecvError> {
        ...
        let _ = self.sender.try_send(Command::Insert { draft: draft.clone(), response_channel: response_sender });
        response_receiver.recv()
    }

    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, RecvError> {
        let (response_sender, response_receiver) = std::sync::mpsc::sync_channel(self.capacity);
        let _ = self.sender.try_send(Command::Get { id: id, response_channel: response_sender});
        response_receiver.recv()
    }
}
