...
impl TicketStoreClient {
    ...
    pub fn update(&self, ticket_patch: TicketPatch) -> Result<(), OverloadedError> {
        let (response_sender, reponse_receiver) =  sync_channel(1);
        self.sender.try_send(Command::Update { patch: ticket_patch, response_channel: response_sender }).map_err(|_| OverloadedError)?;
        Ok(reponse_receiver.recv().unwrap())
    }
}
    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                match patch.title{
                    Some(data) => ticket.title = data.into(),
                    _ => {}
                };
                match patch.description{
                    Some(data) => ticket.description = data.into(),
                    _ => {}
                };
                match patch.status {
                    Some(data) => ticket.status = data,
                    _ => {}
                }
