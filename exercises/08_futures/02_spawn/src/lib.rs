...
pub async fn echoes(first: TcpListener, second: TcpListener) -> Result<(), anyhow::Error> {
    loop{
        let (mut first_stream, _) = first.accept().await?;
        tokio::spawn(async move {
            let (mut reader, mut writer) = first_stream.split();
            let _ = tokio::io::copy(&mut reader, &mut writer).await;
        });
        let (mut second_stream, _) = second.accept().await?;
        tokio::spawn(async move {
            let(mut reader, mut writer) = second_stream.split();
            let _ = tokio::io::copy(&mut reader, &mut writer).await;
        });
    }
}
